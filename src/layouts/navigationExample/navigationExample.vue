  ...
  <div class="flex h-full">
    <div class="unis-navigation h-full z-50 shadow-sm">
      <div :class="{'aside-fold':isFold}" class="aside-unfold transition-all duration-500 relative h-full flex">
        <div
            class="rounded absolute -right-[0.8rem] top-[4.3rem] cursor-pointer box-shadow-xs bg-gray-800   w-[1.8rem] h-[1.8rem] flex items-center justify-center"
        ...
        <div
            ...
            class="parent-level  bg-gray-800  h-full transition-all duration-500 flex  justify-start items-center flex-nowrap flex-col">
          ...
          <div :class="[isFold?'':'overflow-y']"
            ...
            <div v-for="(child, navLinkindex) in navLink" :key="navLinkindex" class="bg-gray-800 h-auto">
              <div v-if="child.groupName"
                   class="group-name min-h-[42px] flex items-center relative pl-[16px] bg-gray-800">
                <div class="text-white text-[10px] z-999 bg-gray-800 transform  scale-[0.83]">{{
                    child.groupName
                  }}
                </div>
              </div>
              <div v-for="(item, childsIndex) in child.children" :key="childsIndex"
                   class="item_connect w-full box-border flex flex-wrap justify-center items-center cursor-pointer relative bg-gray-800 transition-all duration-500">
                <div :class="{'active bg-gray-500 bg-opacity-70 transition-all duration-500':navName==item.title}"
                     class="item_box rounded w-full bg-gray-800 relative transition-all duration-500 flex   items-center justify-start font-base text-sm min-h-[42px] box-border z-9999 mb-1
                ...
                <div v-if="item.children && item.children.length>0"
                  ...
                  <div v-for="nav in item.children"
                    ...
                    <div
                        ...
                        class="item-childs-link justify-between pr-2 flex rounded  items-center text-sm min-h-[40px] box-border pl-[10px] bg-gray-800
