<template>
  <section id="mobile-first bg-indigo  ">

    <div
        class=" mt-16 mb-12 border-b border-gray-100 xl:mb-0 demo-sm:block dark:border-gray-800 h-20 overflow-x-hidden overflow-y-visible">
      <div class="mb-[-3px] flex max-w-7xl mx-auto px-6 sm:px-8 md:px-10">
        <div class="flex items-start flex-none ml-[40rem] w-32">
          <div class="flex flex-col items-center ml-[-2px]">
            <div :class="[dragSimulationWindow > 1 ? 'bg-secondary-600 ml-[-2px]' :'bg-gray-100  dark:bg-primary-800']"
                 class="w-px h-14  "></div>
            <div
                :class="[dragSimulationWindow > 1 ? 'bg-secondary-700 ml-[-2px] ring-indigo-600' :'bg-gray-100 dark:bg-primary-800']"
                class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full  "
            ></div>
          </div>
          <div
              :class="[dragSimulationWindow > 1 ? 'bg-primary-200 text-indigo-600 dark:ring-indigo-600  dark:bg-primary-2000' :'']"
              class="ml-1.5 rounded font-mono text-[0.625rem]    px-3 py-2 ring-1 ring-inset dark:ring-0  dark:text-white  bg-primary-300 dark:bg-primary-800 dark:highlight-white/5">
            sm
          </div>
        </div>
        <div class="flex items-start flex-none w-64">
          <div class="flex flex-col items-center ml-[-2px]">
            <div :class="[dragSimulationWindow > 2 ? 'bg-secondary-600 ml-[-2px]' :'bg-gray-100 dark:bg-primary-800']"
                 class="w-px h-14 "></div>
            <div
                :class="[dragSimulationWindow > 2 ? 'bg-secondary-700 ml-[-2px] ring-indigo-600' :'bg-gray-100 dark:bg-primary-800']"
                class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1  "></div>
          </div>
          <div
              :class="[dragSimulationWindow > 2 ? 'bg-primary-200 text-indigo-600 dark:ring-indigo-600  dark:bg-primary-2000' :'bg-gray-100 ring-gray-100']"
              class="ml-1.5 rounded font-mono text-[0.625rem] leading-6  px-3 py-2 ring-1 ring-inset dark:ring-0  dark:text-white bg-gray-100 ring-gray-100 dark:bg-primary-800 dark:highlight-white/5">
            md
          </div>
        </div>
        <div class="flex items-start flex-none w-64">
          <div class="flex flex-col items-center ml-[-2px]">
            <div :class="[dragSimulationWindow > 3 ? 'bg-secondary-600 ml-[-2px]' :'bg-gray-100 dark:bg-primary-800']"
                 class="w-px h-14"></div>
            <div
                :class="[dragSimulationWindow > 3 ? 'bg-secondary-700 ml-[-2px] ring-indigo-600' :'bg-gray-100 dark:bg-primary-800']"
                class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1  "></div>
          </div>
          <div
              :class="[dragSimulationWindow > 3 ? 'bg-primary-200 text-indigo-600 dark:ring-indigo-600  dark:bg-primary-2000' :'bg-gray-100 ring-gray-100']"
              class="ml-1.5 rounded font-mono text-[0.625rem] leading-6  px-3 py-2 ring-1 ring-inset dark:ring-0  dark:text-white bg-gray-100 ring-gray-100 dark:bg-primary-800 dark:highlight-white/5">
            lg
          </div>
        </div>
        <div class="flex items-start flex-none w-64">
          <div class="flex flex-col items-center ml-[-2px]">
            <div class="w-px h-14 bg-gray-100 dark:bg-primary-800"></div>
            <div
                class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1 bg-white ring-gray-500/[0.15] dark:bg-gray-900 dark:ring-gray-700"></div>
          </div>
          <div
              class="ml-1.5 rounded font-mono text-[0.625rem] leading-6  px-3 py-2 ring-1 ring-inset dark:ring-0 bg-gray-100 ring-gray-100 dark:bg-primary-800 dark:highlight-white/5">
            xl
          </div>
        </div>
        <div class="flex items-start flex-none">
          <div class="flex flex-col items-center ml-[-2px]">
            <div class="w-px h-14 bg-gray-100 dark:bg-primary-800"></div>
            <div
                class="mt-[3px] w-[5px] h-[5px] shadow-sm rounded-full ring-1 bg-white ring-gray-500/[0.15] dark:bg-gray-900 dark:ring-gray-700"></div>
          </div>
          <div
              class="ml-1.5 rounded font-mono text-[0.625rem] leading-6  px-3 py-2 ring-1 ring-inset dark:ring-0 bg-gray-100 ring-gray-100 dark:bg-primary-800 dark:highlight-white/5">
            2xl
          </div>
        </div>
      </div>
    </div>
    <div class="relative pt-10 xl:pt-0 demo-sm:-mt-2.5">
      <div class="absolute  top-[86px]  left-0 right-0 bottom-0  bg-grid-slate-l00 "></div>
      <div
          class="absolute   inset-x-0 h-[37.5rem] bg-grid-gray-900/[0.04] bg-top [mask-image:linear-gradient(0deg,transparent,black)]
          dark:bg-grid-gray-100/[0.03]
          dark:bg-[center_top_-1px] dark:border-t dark:border-gray-700 top-[86px] "></div>
      <div class="sm:px-2 demo-sm:-mt-24 xl:mt-0  h-full ">
        <div class="relative top-0  h-full ">
          <div class="max-w-7xl mx-auto sm:px-6 md:px-8  h-full ">
            <div class="relative h-full w-full">
              <div class="w-full absolute inset-x-0 h-full " style=" width: calc(100% + 20px) ">
                <input
                    v-model="value"
                    class="tick-slider-input drag-input"
                    data-handle-size="18"
                    data-max-label-id="sizeLabelMax"
                    data-min-label-id="sizeLabelMin"
                    data-progress-id="sizeProgress"
                    data-tick-id="sizeTicks"
                    data-tick-step="25"
                    data-value-id="sizeValue"
                    max="100"
                    min="0"
                    step="1"
                    type="range"
                    @input="handleInput"

                />
              </div>

              <div :style="{ width: width + 'px'  }"
                   class="shadow-sm sm:rounded-xl  rounded-lg  demo-sm:min-w-0 demo-sm:max-w-none  h-full  "
              >
                <div class="sm:rounded-xl border border-gray-200 dark:border-none  rounded-lg shadow-xl ">
                  <div
                      class="rounded-t-lg rounded-r-lg border-gray-400 dark:border-none bg-gradient-to-b from-white to-[#FBFBFB] dark:bg-none dark:bg-primary-800 dark:highlight-white/10     ">
                    <div class="py-2.5 grid items-center px-4 gap-8"
                         style="grid-template-columns: 7.125rem 1fr 7.125rem;">
                      <div class="flex items-center">
                        <div class="w-2.5 h-2.5 rounded-full bg-[#EC6A5F]"></div>
                        <div class="ml-1.5 w-2.5 h-2.5 rounded-full bg-[#F4BF50]"></div>
                        <div class="ml-1.5 w-2.5 h-2.5 rounded-full bg-[#61C454]"></div>
                        <svg class="ml-4 flex-none text-gray-400 dark:text-gray-500" fill="none" height="24"
                             width="24">
                          <path d="m15 7-5 5 5 5" stroke="currentColor" stroke-linecap="round"
                                stroke-linejoin="round" stroke-width="2"></path>
                        </svg>
                        <svg class="ml-2 flex-none text-gray-400 dark:text-gray-500" fill="none" height="24"
                             width="24">
                          <path d="m10 7 5 5-5 5" stroke="currentColor" stroke-linecap="round"
                                stroke-linejoin="round" stroke-width="2"></path>
                        </svg>
                      </div>
                      <div>
                        <div
                            class="bg-gray-100 rounded  font-medium text-xs leading-6 py-[5px] flex items-center justify-center dark:border-none border border-gray-200 ring-inset ring-gray-900/5 mx-auto w-4/5 dark:bg-primary-900 dark:text-primary-200 ">
                          <svg class="text-gray-300 w-3.5 h-3.5 mr-1.5 dark:text-gray-500" fill="currentColor"
                               viewBox="0 0 20 20">
                            <path clip-rule="evenodd"
                                  d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z"
                                  fill-rule="evenodd"></path>
                          </svg>
                          unis-ui.com
                        </div>
                      </div>
                      <div class="flex justify-end">
                        <svg class="text-gray-400 dark:text-gray-500" fill="none" height="24"
                             width="24">
                          <path
                              d="M12.5 6a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM12.5 12a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM18.5 6a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM18.5 12a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM6.5 6a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM6.5 12a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM12.5 18a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM18.5 18a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0ZM6.5 18a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0Z"
                              stroke="currentColor" stroke-linecap="round"
                              stroke-linejoin="round"></path>
                        </svg>
                      </div>
                    </div>
                    <div class="grid grid-cols-3 text-xs leading-5 overflow-hidden">
                      <div
                          class="pointer-events-none select-none bg-gray-100 text-gray-400 rounded-tr  dark:border-none border border-gray-900/5 px-4 py-1.5 -mb-px -ml-px flex items-center justify-center space-x-2 dark:bg-primary-900 dark:text-primary-500">
                        <svg class="flex-none text-gray-300 dark:text-gray-500" fill="currentColor" height="10"
                             width="17">
                          <path clip-rule="evenodd"
                                d="M8.5 0C6.233 0 4.817 1.111 4.25 3.334c.85-1.112 1.842-1.528 2.975-1.25.647.158 1.109.618 1.62 1.127C9.68 4.041 10.643 5 12.75 5c2.267 0 3.683-1.111 4.25-3.333-.85 1.111-1.841 1.528-2.975 1.25-.647-.159-1.109-.619-1.62-1.128C11.57.96 10.607 0 8.5 0ZM4.25 5C1.983 5 .567 6.111 0 8.334c.85-1.112 1.842-1.528 2.975-1.25.647.158 1.109.618 1.62 1.127C5.43 9.041 6.393 10 8.5 10c2.267 0 3.684-1.11 4.25-3.333-.85 1.111-1.842 1.528-2.975 1.25-.647-.159-1.109-.619-1.62-1.128C7.32 5.96 6.357 5 4.25 5Z"
                                fill-rule="evenodd"></path>
                        </svg>
                        <div class="truncate">UNIS-UI - Official Tailwind CSS Components
                        </div>
                      </div>
                      <div
                          class="pointer-events-none select-none text-gray-900 font-medium px-4 py-1.5 flex items-center justify-center space-x-2 dark:text-gray-200">
                        <svg class="flex-none text-indigo-600 dark:text-gray-400" fill="currentColor" height="14"
                             width="15">
                          <path
                              d="M6.541 11.753a1.803 1.803 0 0 1-.485 1.277c-.241.253-.552.426-.89.497-.34.07-.691.034-1.01-.103a1.736 1.736 0 0 1-.776-.67 1.79 1.79 0 0 1-.272-1c.004-.306.086-.604.239-.866.152-.262.37-.48.63-.628-.01.047.039-.024 0 0l.797-.723a3.759 3.759 0 0 0 .988-2.535c0-1.28-.734-2.581-1.788-3.262.04.024-.015-.041 0 0a1.72 1.72 0 0 1-.63-.628 1.766 1.766 0 0 1-.238-.865A1.802 1.802 0 0 1 3.592.97c.24-.253.55-.426.89-.496.338-.07.69-.035 1.008.102.319.139.59.372.776.67.187.298.282.647.272 1a3.77 3.77 0 0 0 1.006 2.552l.35.35c.14.125.287.241.44.35.265.143.489.36.644.625a1.73 1.73 0 0 1-.645 2.381c.015-.03-.027.016 0 0a3.89 3.89 0 0 0-1.296 1.393 4.007 4.007 0 0 0-.496 1.856Zm1.921-9.512c0 .348.101.69.29.979.188.29.457.515.77.648a1.678 1.678 0 0 0 1.872-.382 1.803 1.803 0 0 0 .372-1.919 1.752 1.752 0 0 0-.632-.79 1.685 1.685 0 0 0-2.168.22c-.322.33-.503.778-.504 1.244Zm1.718 7.751c-.34 0-.672.104-.954.297a1.752 1.752 0 0 0-.633.79A1.802 1.802 0 0 0 8.966 13a1.679 1.679 0 0 0 1.871.382c.314-.134.582-.36.77-.65a1.796 1.796 0 0 0-.214-2.223 1.684 1.684 0 0 0-1.214-.516Zm4.393-2.995c0-.348-.1-.688-.29-.978a1.727 1.727 0 0 0-.77-.649 1.677 1.677 0 0 0-.993-.1 1.7 1.7 0 0 0-.878.482 1.803 1.803 0 0 0-.373 1.92c.13.32.35.596.633.79a1.684 1.684 0 0 0 2.167-.22c.323-.331.504-.779.504-1.245Z"></path>
                          <path
                              d="M2.147 5.237c-.34 0-.672.103-.954.296a1.753 1.753 0 0 0-.633.79 1.803 1.803 0 0 0 .373 1.92c.24.245.545.413.878.48.333.069.679.034.993-.099.314-.133.582-.359.77-.648a1.795 1.795 0 0 0-.214-2.223 1.714 1.714 0 0 0-1.213-.516Z"></path>
                        </svg>
                        <div class="truncate">UNIS-UI - Find a trip that suits you</div>
                      </div>
                      <div
                          class="pointer-events-none select-none bg-gray-100 text-gray-400 rounded-tl  dark:border-none border border-gray-900/5 pl-4 pr-8 py-1.5 -mb-px -mr-4 flex items-center justify-center space-x-2 dark:bg-primary-900 dark:text-primary-500">
                        <svg class="flex-none text-gray-300 dark:text-gray-500" fill="currentColor" height="16"
                             width="15">
                          <path
                              d="m2.973 9.822 9.154-3.056c-.183-1.144-.314-1.908-.465-2.491-.162-.627-.291-.795-.342-.853a1.785 1.785 0 0 0-.643-.467c-.071-.03-.27-.102-.917-.063-.684.042-1.581.181-3.003.406-1.42.225-2.318.37-2.98.542-.627.162-.796.292-.854.342a1.792 1.792 0 0 0-.466.643c-.03.071-.102.271-.063.918.041.683.181 1.581.406 3.002.063.399.12.755.173 1.077Z"></path>
                          <path clip-rule="evenodd"
                                d="M.447 9.117C.012 6.367-.206 4.993.265 3.89a4.166 4.166 0 0 1 1.09-1.5C2.26 1.6 3.633 1.382 6.382.946c2.75-.436 4.125-.653 5.229-.182a4.164 4.164 0 0 1 1.5 1.09c.79.904 1.007 2.278 1.442 5.028.436 2.75.653 4.124.182 5.227a4.164 4.164 0 0 1-1.09 1.5c-.903.79-2.278 1.008-5.028 1.443-2.749.436-4.124.653-5.227.182a4.166 4.166 0 0 1-1.5-1.09C1.1 13.241.883 11.867.447 9.117Zm4.85 4.882c.735-.044 1.684-.193 3.087-.415 1.404-.222 2.351-.374 3.066-.56.691-.179 1.01-.354 1.216-.534a2.68 2.68 0 0 0 .7-.964c.108-.252.176-.609.133-1.322-.045-.736-.193-1.685-.416-3.088-.222-1.404-.373-2.352-.559-3.066-.18-.692-.354-1.01-.534-1.216a2.678 2.678 0 0 0-.964-.7c-.252-.108-.609-.176-1.323-.133-.736.044-1.684.193-3.088.415-1.403.223-2.35.374-3.065.56-.692.179-1.01.354-1.216.534a2.678 2.678 0 0 0-.7.964c-.108.251-.176.609-.133 1.322.045.737.193 1.685.415 3.088.223 1.404.374 2.352.56 3.066.179.692.354 1.01.534 1.216.265.303.594.543.964.7.252.109.608.176 1.323.133Z"
                                fill-rule="evenodd"></path>
                        </svg>
                        <div class="truncate">UNIS-UI UI – Unstyled, fully accessible UI
                          components
                        </div>
                      </div>
                    </div>
                  </div>
                  <div
                      class="relative bg-white border-t rounded-b-lg
            ring-primary-900/5
            dark:highlight-white/5
            dark:ring-2   dark:bg-primary-800 dark:border-primary-900 ">
                    <slot></slot>
                    <iframe :src="src" :style="{'height':height}" class="w-full "
                            style="pointer-events: auto;"
                            title="Mobile-first Demo"></iframe>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>
  </section>
</template>
<script lang="ts" setup>
</script>
